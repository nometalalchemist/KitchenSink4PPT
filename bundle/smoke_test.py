"""Smoke test for the KitchenSink4PPT .mcpb bundle.
...
Checks, in order:
  ...
  4. If uvx is available (PATH or next to this interpreter), launches
     `uvx kitchensink4ppt==<ver>` and performs a real MCP stdio
     initialize handshake, proving the launch command Claude Desktop
     will run actually starts the server.

Usage: python -X utf8 smoke_test.py [path/to/kitchensink4ppt.mcpb]
...
def main() -> int:
    mcpb_path = Path(sys.argv[1]) if len(sys.argv) > 1 else HERE / "kitchensink4ppt.mcpb"
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        check("name", m.get("name") == "kitchensink4ppt", m.get("name"))
        srv = m.get("server", {})
        ...
        ver = m.get("version", "")
        expected_arg = f"kitchensink4ppt=={ver}"
        check(
