...
def test_an_empty_lockfile_is_not_read_as_a_dead_holder(tmp_path, monkeypatch):
    ...
    with pytest.raises(MutationLockTimeout):
        safesave._acquire_lockfile(lock, "contended.pptx")
...
def test_an_unreadable_lockfile_is_reclaimed_once_the_grace_expires(tmp_path):
    ...
    assert safesave._acquire_lockfile(lock, "abandoned.pptx") is True
...
def test_a_second_acquisition_by_this_process_is_re_entrant(tmp_path):
    lock = tmp_path / safesave.LOCK_FILE_NAME
    assert safesave._acquire_lockfile(lock, "t.pptx") is True
    assert safesave._acquire_lockfile(lock, "t.pptx") is False
    safesave._release_lockfile(lock)
...
def test_a_matching_creation_time_keeps_a_live_holder_protected(
    ...
    with pytest.raises(MutationLockTimeout, match="4321"):
        safesave._acquire_lockfile(lock, "t.pptx")
    assert lock.exists()
    lock.unlink()
...
WRITER = textwrap.dedent("""\
    ...
    from kitchensink4ppt.core.safesave import write_lock
...
def test_four_concurrent_writers_never_overlap(tmp_path, make_deck):
    """The xlsx contention repro: four or more real processes racing for the
    same presentation's write lock.
...
CONTENDER = textwrap.dedent("""\
    ...
    sys.path.insert(0, sys.argv[1])
    from kitchensink4ppt.core import safesave
    from kitchensink4ppt.core.safesave import write_lock, MutationLockTimeout
...
def test_four_concurrent_writers_cannot_steal_a_half_written_lock(
    tmp_path, make_deck
):
    ...
    processes race for the same presentation.
