"""Is a newer KitchenSink4PPT published? A quiet, opt-out startup check.
...
- The result surfaces as ONE line in diagnose() and nowhere else. No
  startup banner, no per-call nagging.
- KS4P_NO_UPDATE_CHECK=1 (or "true") disables the whole thing: no network
...
OPT_OUT_ENV = "KS4P_NO_UPDATE_CHECK"
...
def start_background_check() -> threading.Thread | None:
    ...
    try:
        thread = threading.Thread(
            target=run_check, name="ks4p-update-check", daemon=True,
        )
        thread.start()
        return thread
    except Exception:
