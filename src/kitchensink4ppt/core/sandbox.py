"""Opt-in path sandboxing governed by the KS4P_ALLOWED_ROOTS environment
variable.

When KS4P_ALLOWED_ROOTS is unset (or empty), nothing here restricts anything:
...
plants content, so presentation opens, image reads, data imports, and backup
...
ENV_VAR = "KS4P_ALLOWED_ROOTS"


class SandboxViolation(PptMcpError):
    """A path resolved outside the allowed roots; nothing was read or
    written. KS4P_ALLOWED_ROOTS governs which directories are allowed."""
...
def check_path(path: str | os.PathLike, purpose: str = "access") -> str:
    ...
    operation in the error text ("open presentation", "export image output")."""
    p = os.fspath(path)
